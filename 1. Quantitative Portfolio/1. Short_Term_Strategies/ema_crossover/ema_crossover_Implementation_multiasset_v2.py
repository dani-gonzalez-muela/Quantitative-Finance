"""
EMA Crossover -- Multi-Asset Implementation v2
==============================================
Reads canonical params + per-ticker trade CSVs produced by the Backtest (Phase 2).
Runs multiple sizing variants using shared.implementations, prints comparison,
and saves combined_equity.csv for each variant.
...
Outputs:
  ...
  results/ema_crossover_v2_implementations.json   (comparison summary + selected_variant)
"""
...
CANON_PATH  = os.path.join(RESULTS_DIR, 'ema_crossover_v2_canonical_params.json')
BONF_PATH   = os.path.join(RESULTS_DIR, 'ema_crossover_v2_bonferroni_results.json')
...
def load_trades(basket_name, ticker, bonferroni=False):
    if bonferroni:
        path = os.path.join(TRADES_DIR, f'ema_crossover_v2_trades_bonferroni_{basket_name}_{ticker}.csv')
    else:
        path = os.path.join(TRADES_DIR, f'ema_crossover_v2_trades_{basket_name}_{ticker}.csv')
    if not os.path.exists(path): return None
...
def build_ticker_equities(basket_name, ticker, sleeve_cap):
    ...
    sleeve_cap = STARTING_CAPITAL / N_passing_baskets / N_tickers_in_basket
    """
    df5 = load_5min(ticker)
    if df5 is None or df5.index.normalize().nunique() < MIN_DAYS:
        return {}
    ...
    # 1. Simple bet -- 85% of sleeve equity per trade
    try:
        r = simple_bet(trades, bet_size=0.85, starting_capital=sleeve_cap, slippage=SLIPPAGE)
    ...
    # 2. Intraday asset vol targeting -- paper's method (2% daily target, 4x max leverage)
    try:
        r = intraday_asset_vol(trades, daily_prices, target_vol=0.02, lookback=14,
                               max_leverage=4.0, starting_capital=sleeve_cap, cost_per_share=SLIPPAGE)
        eq = build_daily_equity(trades, r['equity_curve'], sleeve_cap)
    ...
    # 3. Strategy vol targeting -- scale by strategy's own return vol (10% ann, 2x max)
    try:
        r = vol_targeting(trades, target_vol=0.10, lookback=60, max_leverage=2.0,
                          starting_capital=sleeve_cap, slippage=SLIPPAGE)
        eq = build_daily_equity(trades, r['equity_curve'], sleeve_cap)
...
# -- Combine ticker equities into basket equity --------------------------------

def combine_equities(equity_dict, sleeve_cap):
    """Equal-weight a dict of {ticker: daily_equity_Series} into one basket equity."""
    if not equity_dict: return None
    all_dates = sorted(set().union(*[eq.index for eq in equity_dict.values()]))
    aligned = {}
    for ticker, eq in equity_dict.items():
        s = eq.copy()
        s.index = pd.to_datetime(s.index)
        aligned[ticker] = s.reindex(pd.to_datetime(all_dates)).ffill().bfill()
    # Normalize each ticker to start at sleeve_cap, then equal-weight
    n = len(aligned)
    combined = sum(s / s.iloc[0] * sleeve_cap for s in aligned.values()) / n
...
def main():
    ...
    # Load Bonferroni rescued tickers {ticker -> {basket, fast_ema, slow_ema, stop_val}}
    rescued = {}
        ...
        for basket_name, results in bonf.get('baskets', {}).items():
            for r in results:
                if r.get('pass'):
                    rescued[r['ticker']] = {
                        'basket': basket_name,
                        'fast_ema': r['fast_ema'], 'slow_ema': r['slow_ema'], 'stop_val': r['stop_val'],
                    }
    else:
    ...
    # Accumulate per-variant equity curves (one per instrument, equal-weight all)
    ...
    for basket_name, params in passing.items():
        ...
        fast = params['fast_ema']; slow = params['slow_ema']; sv = params['stop_val']
        print(f"\n{'='*60}")
        print(f"Basket: {basket_name} | fast={fast} slow={slow} sv={sv}")
    ...
    json_path = os.path.join(RESULTS_DIR, 'ema_crossover_v2_implementations.json')
    ...
    print("\nReview the table above and set combined_equity.csv to your preferred variant.")
