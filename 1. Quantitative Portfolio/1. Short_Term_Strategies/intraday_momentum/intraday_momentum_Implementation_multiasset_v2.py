"""
Intraday Momentum -- Multi-Asset Implementation v2
===================================================
Reads canonical params + per-ticker trade CSVs produced by the Backtest (Phase 1
basket-passing tickers + Phase 2 Bonferroni-rescued tickers).
...
Outputs:
  ...
  results/intraday_momentum_v2_implementations.json   (comparison summary + selected_variant)
"""
...
CANON_PATH  = os.path.join(RESULTS_DIR, 'intraday_momentum_v2_canonical_params.json')
BONF_PATH   = os.path.join(RESULTS_DIR, 'intraday_momentum_v2_bonferroni_results.json')
...
def load_trades(basket_name, ticker, bonferroni=False):
    if bonferroni:
        path = os.path.join(TRADES_DIR, f'intraday_momentum_v2_trades_bonferroni_{basket_name}_{ticker}.csv')
    else:
        path = os.path.join(TRADES_DIR, f'intraday_momentum_v2_trades_{basket_name}_{ticker}.csv')
    if not os.path.exists(path): return None
...
def main():
    ...
    # Load Bonferroni rescued tickers {ticker -> {basket, lookback, vol_mult}}
    rescued = {}
        ...
        for basket_name, results in bonf.get('baskets', {}).items():
            for r in results:
                if r.get('pass'):
                    rescued[r['ticker']] = {
                        'basket': basket_name,
                        'lookback': r['lookback'], 'vol_mult': r['vol_mult'],
                    }
    else:
        print("WARNING: bonferroni results not found -- only basket-passing tickers included.")

    # isinstance guard: canonical.json may carry non-dict metadata keys (e.g. a
    # "_run_info" string) alongside basket entries -- guard against calling
    # .get() on a non-dict value.
    ...
    for basket_name, params in passing.items():
        ...
        lb = params['lookback']; vm = params['vol_mult']
        print(f"\n{'='*60}")
        print(f"Basket: {basket_name} | lookback={lb} vol_mult={vm}")
    ...
    json_path = os.path.join(RESULTS_DIR, 'intraday_momentum_v2_implementations.json')
