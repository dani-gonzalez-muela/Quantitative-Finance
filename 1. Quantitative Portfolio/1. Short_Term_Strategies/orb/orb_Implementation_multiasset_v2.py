"""
ORB (Opening Range Breakout) -- Multi-Asset Implementation v2
===============================================================
Reads canonical params + per-ticker trade CSVs produced by the Backtest (Phase 1 + Phase 2).
...
Outputs:
  ...
  results/orb_v2_implementations.json             (comparison summary + selected_variant)
"""
...
CANON_PATH  = os.path.join(RESULTS_DIR, 'orb_v2_canonical_params.json')
BONF_PATH   = os.path.join(RESULTS_DIR, 'orb_per_ticker_bonferroni_results.json')
...
def load_trades(basket_name, ticker, bonferroni=False):
    if bonferroni:
        path = os.path.join(TRADES_DIR, f'orb_v2_trades_bonferroni_{basket_name}_{ticker}.csv')
    else:
        path = os.path.join(TRADES_DIR, f'orb_v2_trades_{basket_name}_{ticker}.csv')
    if not os.path.exists(path): return None
...
def build_ticker_equities(basket_name, ticker, sleeve_cap):
    ...
    sleeve_cap = STARTING_CAPITAL / N_total_validated_instruments
    """
    df5 = load_5min(ticker)
    if df5 is None or df5.index.normalize().nunique() < MIN_DAYS:
        return {}
...
def main():
    ...
    # Load Bonferroni rescued tickers {ticker -> {basket, window_size, atr_percent, threshold}}
    rescued = {}
        ...
        for basket_name, results in bonf.get('baskets', {}).items():
            for r in results:
                if r.get('pass'):
                    rescued[r['ticker']] = {
                        'basket': basket_name,
                        'window_size': r['window_size'], 'atr_percent': r['atr_percent'],
    ...
    for basket_name, params in passing.items():
        ...
        ws = params['window_size']; ap = params['atr_percent']; th = params['threshold']
        print(f"\n{'='*60}")
        print(f"Basket: {basket_name} | window={ws}min atr={ap}% thr={th}%")

        basket_impl = {}
        for ticker in tickers:
            print(f"  {ticker}:")
            per_ticker = build_ticker_equities(basket_name, ticker, sleeve_per_instrument)
    ...
    if rescued_only:
        ...
        for ticker, params in rescued_only.items():
            ...
            per_ticker = build_ticker_equities_bonferroni(basket_name, ticker, sleeve_per_instrument)
            for variant, eq in per_ticker.items():
                all_equities[variant][ticker] = eq
                ret = eq.pct_change().dropna()
                sh = sharpe(ret); dd = max_dd(eq); ca = cagr(eq)
    ...
    json_path = os.path.join(RESULTS_DIR, 'orb_v2_implementations.json')
