...
variants using shared.implementations, then selects the highest-Sharpe
variant as the final combined equity for the strategy.

Sizing variants compared:
  1. simple_bet         -- fixed 85% of equity per trade
  2. intraday_asset_vol  -- underlying asset vol targeting (2% daily, 4x max)
...
Outputs:
  ...
  results/overnight_v2_implementations.json       (comparison summary + selected_variant)

All writes are local to this strategy's own results/ folder -- unlike the
prior overnight_bonferroni_intraday.py (now archived), this script does not
write anywhere outside overnight/results/.
"""
...
CANON_PATH  = os.path.join(RESULTS_DIR, 'overnight_v2_canonical_params.json')
BONF_PATH   = os.path.join(RESULTS_DIR, 'overnight_v2_bonferroni_results.json')
...
SLIPPAGE = float(os.environ.get('SLIPPAGE', '0.005'))  # $/share per side; canonical default set 2026-07-10
...
# Must match overnight_Backtest_multiasset_v2.py's BASKETS exactly -- this
# was the source of a basket-definition mismatch bug in the prior version
# (overnight_bonferroni_intraday.py tested a different us_equity_broad
# ticker set than the Backtest script defined). Single source of truth here.
BASKETS = {
...
def load_trades(basket_name, ticker, bonferroni=False):
    if bonferroni:
        path = os.path.join(TRADES_DIR, f'overnight_v2_trades_bonferroni_{basket_name}_{ticker}.csv')
    else:
        path = os.path.join(TRADES_DIR, f'overnight_v2_trades_{basket_name}_{ticker}.csv')
    if not os.path.exists(path): return None
    trades = pd.read_csv(path, parse_dates=['entry_time', 'exit_time'])
    if trades.empty: return None
    # entry_time/exit_time cross DST transitions (mixed -04:00/-05:00 offsets --
    # confirmed present in this strategy's own trade data: after-market close
    # is ~19:55 ET which is EDT in summer, EST in winter). pandas' CSV parser
    # leaves that as an object column of mixed-offset datetime.datetime. Force
    # to a proper tz-aware UTC datetime64 dtype so downstream pd.to_datetime()
    # calls (including shared.implementations.build_daily_equity) don't choke.
...
def _size_ticker(trades, daily_prices, sleeve_cap):
    """Run all 3 sizing variants for one ticker's trades. Shared by the
    basket-passing and Bonferroni-rescued code paths."""
    results = {}
    try:
    ...
    try:
        ...
        print(f"    intraday_asset_vol error: {e}")
    try:
        ...
        print(f"    vol_targeting error: {e}")
    return results


def build_ticker_equities(basket_name, ticker, sleeve_cap):
    """Returns dict {variant_name: daily_equity_Series} for one basket-passing ticker."""
    df5 = load_5min(ticker)
    ...
    return _size_ticker(trades, daily_prices, sleeve_cap)
...
def main():
    ...
    # Load Bonferroni rescued tickers {ticker -> {basket, threshold}}
    rescued = {}
    if os.path.exists(BONF_PATH):
        ...
        for basket_name, results in bonf.get('baskets', {}).items():
            for r in results:
                if r.get('pass'):
                    rescued[r['ticker']] = {'basket': basket_name, 'threshold': r['threshold']}
    else:
        print("WARNING: bonferroni results not found -- only basket-passing tickers included.")

    # canonical contains a '_run_info' string key alongside per-basket dicts --
    # must guard every comprehension with isinstance(p, dict).
    passing = {b: p for b, p in canonical.items() if isinstance(p, dict) and p.get('gate_pass', False)}
    print(f"\nPassing baskets:       {len(passing)}/{sum(1 for v in canonical.values() if isinstance(v, dict))}")
    ...
    # -- Basket-passing tickers (at basket canonical threshold) ------------------
    N_baskets = len(passing)
    for basket_name, params in passing.items():
        tickers = BASKETS[basket_name]
        thr = params['threshold']
        print(f"\n{'='*60}")
        print(f"Basket: {basket_name} | threshold={thr}")
    ...
    # -- Bonferroni rescued tickers (at per-ticker optimal threshold) -----------
    if rescued_only:
        print(f"\n{'='*60}")
    ...
    json_path = os.path.join(RESULTS_DIR, 'overnight_v2_implementations.json')
