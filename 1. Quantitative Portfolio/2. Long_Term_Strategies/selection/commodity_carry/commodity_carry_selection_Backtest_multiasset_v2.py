...
# -*- coding: utf-8 -*-
"""
Commodity Carry — Multi-Asset Selection Backtest v2
Full grid. Canonical = highest median Sharpe. Binomial test. n_boot=500.
Outputs: results/commodity_carry_v2_multiasset_summary.json
"""
import sys, os
...
STRATEGY_NAME="Commodity Carry v2 (Multi-Asset)"; SAVE_NAME="commodity_carry"
...
UNIVERSE_SPECS={"commodities":["GLD","SLV","DBC","USO","UNG","GDX","PDBC"]}
MIN_TICKERS=3
TOP_PCTS=[0.10,0.15,0.25,0.33,0.50,1.00]; REBAL_FREQS=["ME","2ME","QE","6ME","12ME"]
SHORT_WINS=[1,2,3]; LONG_WINS=[3,6,9,12]
CARRY_PARAMS=[(sw,lw) for sw in SHORT_WINS for lw in LONG_WINS if sw<lw]
ALL_COMBOS=list(itertools.product(TOP_PCTS,REBAL_FREQS,CARRY_PARAMS))
...
def load_prices(tickers):
    ...
    for t in tickers:
        ...
        try:
            ...
            df.index=pd.to_datetime(df.index); s=df["close"].dropna()
...
for uni,tickers in UNIVERSE_SPECS.items():
    ...
    else: print(f"  {uni}: SKIP")

def compute_carry_signal(prices, sw, lw, freq):
    monthly=prices.resample(freq).last(); fm=_FREQ_MONTHS.get(freq,1)
    sw2=max(1,int(round(sw/fm))); lw2=max(sw2+1,int(round(lw/fm)))
    sig=pd.DataFrame(index=monthly.index,columns=monthly.columns,dtype=float)
    for i in range(len(monthly)):
        if i<lw2+1: continue
        cs=monthly.iloc[i-1]/monthly.iloc[i-1-sw2]-1
        cl=(monthly.iloc[i-1]/monthly.iloc[i-1-lw2]-1)/lw2
        sig.iloc[i]=cs-cl
    return sig
...
def run_trades(prices,combo):
    top_pct,freq,(sw,lw)=combo; top_n=max(1,round(len(prices.columns)*top_pct))
    try:
        sig=compute_carry_signal(prices,sw,lw,freq)
        tr=generate_trades(prices,sig,top_n,freq)
        return tr if tr is not None and not tr.empty and len(tr)>=max(top_n,2) else pd.DataFrame()
...
def basket_sig3(trades,n_boot=N_BOOT):
    ...
    gaps=pd.to_datetime(cohort.index).to_series().diff().dropna().dt.days
    ann=np.sqrt(365/gaps.median() if len(gaps)>0 else 12)
    sh=float(r.mean()/r.std()*ann)
...
def combo_to_params(c):
    tp,freq,(sw,lw)=c; return {"top_pct":float(tp),"freq":freq,"short_win":int(sw),"long_win":int(lw)}
...
for u,prices in UNIVERSES.items():
    ...
    ng,sh,det=basket_sig3(tr); canon_sh.append(sh); canon_det[u]={**det,"sharpe":sh,"gates":ng}
...
print("\n  Top-10 combos:")
for rk,(ci,med) in enumerate(ranked[:10],1):
    p=combo_to_params(ALL_COMBOS[ci])
    print(f"  {rk:>2}  med={med:>6.3f}  top_pct={p['top_pct']}  sw={p['short_win']}  lw={p['long_win']}  {p['freq']}")
        ...
        bonferroni_rescue(
            ...
            monthly_returns_fn=lambda _t, _c: compute_monthly_returns(
                build_daily_equity_from_trades(
                    ticker_data[_t]["close"], generate_commodity_carry_trades  # TODO: verify fn name(ticker_data[_t], _t, *_c))),
            out_path=os.path.join(RESULTS_DIR, "commodity_carry_v2_bonferroni_results.json"),
        )
    else:
