...
# -*- coding: utf-8 -*-
"""Commodity Carry — Implementation v2 (TYPE 2 SELECTION). Uses build_basket_equity."""
import sys, os
...
STRATEGY_NAME="commodity_carry_v2"; STARTING_CAPITAL=100_000
...
SUMMARY_PATH=os.path.join(RESULTS_DIR,"commodity_carry_v2_multiasset_summary.json")
...
print("="*70); print("COMMODITY CARRY — Implementation v2  (TYPE 2 SELECTION)"); print("="*70)
...
def compute_carry_signal(prices,sw,lw,freq):
    ...
    for i in range(len(monthly)):
        if i<lw2+1: continue
        cs=monthly.iloc[i-1]/monthly.iloc[i-1-sw2]-1; cl=(monthly.iloc[i-1]/monthly.iloc[i-1-lw2]-1)/lw2
        sig.iloc[i]=cs-cl
    return sig
...
print(f"  Sleeve: ${sleeve_cap:,.0f}  allocation={ALLOCATION}")
basket_results={}; sleeve_eq_dict={}

for bname,cfg in passing.items():
    cp=cfg["canonical_params"]; top_pct=float(cp["top_pct"]); freq=cp["freq"]
    sw=int(cp["short_win"]); lw=int(cp["long_win"])
    tickers=UNIVERSE_SPECS.get(bname,cfg.get("instruments",[]))
    prices=load_prices(tickers)
    if prices.empty: print(f"  {bname}: no data"); continue
    top_n=max(1,round(len(prices.columns)*top_pct)); sig=compute_carry_signal(prices,sw,lw,freq)
    trades=generate_trades(prices,sig,top_n,freq)
    print(f"  BASKET: {bname}  top_pct={top_pct} sw={sw} lw={lw} {freq}  {len(trades)} trades")
...
if sleeve_eq_dict:
    ...
    cst=compute_stats(combined); print(f"  Sharpe={cst['sharpe']:.4f}  CAGR={cst['cagr_pct']:.2f}%  MaxDD={cst['max_dd_pct']:.2f}%")
...
jp=os.path.join(RESULTS_DIR,"commodity_carry_v2_implementations_multiasset.json")
