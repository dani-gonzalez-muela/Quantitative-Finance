# -*- coding: utf-8 -*-
"""
Commodity Carry — Multi-Asset Selection Backtest
================================================
Adapts commodity_carry_selection_backtest.py to ETF universes.
Original signal: carry = 1m_return - 3m_return/3  (roll yield differential proxy)
                 Rank by carry, long top TOP_N commodities.
Multiasset adaptation: same carry signal on commodity ETFs.
...
----
  ...
  SHORT_WINS  = [1, 2, 3]   (months — short lookback for carry numerator)
  LONG_WINS   = [3, 6, 9, 12]  (months — long lookback for carry denominator)
  (Only combos where SHORT_WIN < LONG_WIN are valid)
  Note: this replaces MOM_WINDOWS with a 2D carry parameter sweep.

Outputs
-------
  results/commodity_carry_multiasset_grid.csv
  results/commodity_carry_multiasset_summary.json
"""
...
STRATEGY_NAME    = "Commodity Carry (Multi-Asset)"
SAVE_NAME        = "commodity_carry_multiasset"
...
TOP_PCTS    = [0.10, 0.15, 0.25, 0.33, 0.50, 1.00]
REBAL_FREQS = ["ME", "2ME", "QE", "6ME", "12ME"]
# Carry parameter sweep: (short_win, long_win) — analogous to MOM_WINDOWS
SHORT_WINS  = [1, 2, 3]
LONG_WINS   = [3, 6, 9, 12]
CARRY_PARAMS = [(sw, lw) for sw in SHORT_WINS for lw in LONG_WINS if sw < lw]

_FREQ_MONTHS = {"ME": 1, "2ME": 2, "QE": 3, "6ME": 6, "12ME": 12}

print("=" * 75)
print("COMMODITY CARRY — Multi-Asset Backtest")
print("=" * 75)
print(f"Period        : {START_DATE} → {END_DATE}")
print(f"Carry params  : {CARRY_PARAMS}")
print()
...
def compute_carry_signal(prices_wide, short_win, long_win, rebalance_freq):
    """
    Carry proxy (same as original backtest):
        carry = short_win_return - long_win_return / long_win
    Uses monthly returns at the rebalance frequency.
    """
    monthly = prices_wide.resample(rebalance_freq).last()
    months_per_period = _FREQ_MONTHS.get(rebalance_freq, 1)
    sw = max(1, int(round(short_win / months_per_period)))
    lw = max(sw + 1, int(round(long_win / months_per_period)))

    signal_df = pd.DataFrame(index=monthly.index, columns=monthly.columns, dtype=float)
    for i in range(len(monthly)):
        if i < lw + 1:
            continue
        carry_short = monthly.iloc[i-1] / monthly.iloc[i-1-sw] - 1
        carry_long  = (monthly.iloc[i-1] / monthly.iloc[i-1-lw] - 1) / lw
        signal_df.iloc[i] = carry_short - carry_long
    return signal_df
...
def run_single(prices, top_pct, freq, short_win, long_win):
    n = len(prices.columns); top_n = max(1, round(n * top_pct))
    try:
        sig    = compute_carry_signal(prices, short_win, long_win, freq)
...
# ── Grid search ───────────────────────────────────────────────────────────────
all_combos = list(product(TOP_PCTS, REBAL_FREQS, CARRY_PARAMS))
...
print(f"(carry params: {len(CARRY_PARAMS)} × top_pcts: {len(TOP_PCTS)} × rebal_freqs: {len(REBAL_FREQS)})")
print(f"{'='*75}\n")

results_raw = []
for idx, (top_pct, freq, (sw, lw)) in enumerate(all_combos, 1):
    row = {"top_pct": top_pct, "freq": freq, "short_win": sw, "long_win": lw}
    sharpes = []
    for uni_name, prices in UNIVERSES.items():
        s = run_single(prices, top_pct, freq, sw, lw)
        row[uni_name] = s
...
for rank, row in grid_df_sorted.head(20).iterrows():
    ...
    print(f"{rank:>4}  top_pct={row['top_pct']:.2f}  short={int(row['short_win'])}  "
          f"long={int(row['long_win'])}  {row['freq']:>5}  med={row['median_sharpe']:>6.3f}  [{uni_vals}]")

os.makedirs(RESULTS_DIR, exist_ok=True)
col_order = ["top_pct","top_n","freq","short_win","long_win","median_sharpe","min_sharpe","n_positive"] + uni_names
grid_df[[c for c in col_order if c in grid_df.columns]].sort_values(
    "median_sharpe", ascending=False).to_csv(os.path.join(RESULTS_DIR, "commodity_carry_multiasset_grid.csv"), index=False)
...
summary = {
    ...
    "best_params": {
        ...
        "short_win": int(best["short_win"]), "long_win": int(best["long_win"]),
        "freq": best["freq"],
...
with open(os.path.join(RESULTS_DIR, "commodity_carry_multiasset_summary.json"), "w") as f:
...
def get_period_returns(prices, top_pct, freq, sw, lw):
    n = len(prices.columns); top_n = max(1, round(n*top_pct))
    try:
        sig    = compute_carry_signal(prices, sw, lw, freq)
        trades = generate_trades(prices, sig, top_n, freq)
        if trades.empty or len(trades) < max(top_n, 2):
            return None, None
        cohort = trades.groupby("entry_time")["pct_return_gross"].mean().reset_index()
...
for _, _rr in _top3.iterrows():
    _tp = float(_rr["top_pct"]); _freq = _rr["freq"]; _sw = int(_rr["short_win"]); _lw = int(_rr["long_win"])
    print(f"\n  -- top_pct={_tp:.2f}  short={_sw}  long={_lw}  {_freq} --")
    print("  {:<15}  {:>7}  {:>7}  {:>7}  {:>8}  {:>7}  Score".format("Universe","Sharpe","t-stat","t-p","boot5%","perm-p"))
    print("  "+"-"*71)
    for _uni, _prices in UNIVERSES.items():
        _ck = (_tp, _freq, _sw, _lw, _uni)
        if _ck not in _pret_cache: _pret_cache[_ck] = get_period_returns(_prices, _tp, _freq, _sw, _lw)
