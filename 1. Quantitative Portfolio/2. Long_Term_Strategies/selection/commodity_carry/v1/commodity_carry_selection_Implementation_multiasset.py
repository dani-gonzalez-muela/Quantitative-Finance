...
# -*- coding: utf-8 -*-
"""
Commodity Carry — Multi-Asset Implementation
=============================================
Canonical: top_pct=0.50, short_win=2, long_win=12, freq=ME
Basket   : commodities

Carry signal: carry = (N_short_return) - (N_long_return / long_win)
  carry_short = monthly.iloc[i-1] / monthly.iloc[i-1-sw] - 1
  carry_long  = (monthly.iloc[i-1] / monthly.iloc[i-1-lw] - 1) / lw

Outputs: results/commodity_carry_multiasset_daily_equity/*.csv
         results/commodity_carry_implementations_multiasset.json
"""
import sys, os
...
SUMMARY_PATH = os.path.join(RESULTS_DIR, "commodity_carry_multiasset_summary.json")
...
print("="*70); print("COMMODITY CARRY — Multi-Asset Implementation"); print("="*70)

with open(SUMMARY_PATH) as f: summary = json.load(f)
best = summary["best_params"]
CANON_TOP_PCT = float(best["top_pct"])
CANON_SW      = int(best["short_win"])
CANON_LW      = int(best["long_win"])
CANON_FREQ    = best["freq"]
PASSING_BASKETS = list(summary["universes"].keys())
print(f"\nCanonical: top_pct={CANON_TOP_PCT}, sw={CANON_SW}, lw={CANON_LW}, freq={CANON_FREQ}")
...
for basket_name in PASSING_BASKETS:
    ...
    sig = compute_carry_signal(prices, CANON_SW, CANON_LW, CANON_FREQ)
...
out = {"strategy":STRATEGY_NAME,"period":f"{START_DATE} → {END_DATE}","tc_bps_one_way":TC_BPS_OW,
       "canonical_params":{"top_pct":CANON_TOP_PCT,"short_win":CANON_SW,"long_win":CANON_LW,"freq":CANON_FREQ},
       "sleeves":{b:{v:d["metrics"] for v,d in vd.items()} for b,vd in sleeve_results.items()},
       "combined":combined_results}
jp = os.path.join(RESULTS_DIR, "commodity_carry_implementations_multiasset.json")
