...
# -*- coding: utf-8 -*-
"""Commodity Trend — Multi-Asset Selection Backtest v2
Full grid. Canonical=highest median Sharpe across baskets. Binomial test. n_boot=500.
Outputs: results/commodity_trend_v2_multiasset_summary.json
"""
import sys, os
...
STRATEGY_NAME="Commodity Trend v2 (Multi-Asset)"; SAVE_NAME="commodity_trend"
...
UNIVERSE_SPECS={"commodities":["GLD","SLV","DBC","USO","UNG","GDX","PDBC"],"real_assets":["VNQ","VNQI","AMLP","XLRE"]}
MIN_TICKERS=3
TOP_PCTS=[0.10,0.15,0.25,0.33,0.50,1.00]; REBAL_FREQS=["ME","2ME","QE","6ME","12ME"]; MOM_WINDOWS=[3,6,9,12]
...
def run_trades(prices,combo):
    top_pct,freq,mom_win=combo; top_n=max(1,round(len(prices.columns)*top_pct))
    try:
        sig=compute_momentum(prices,mom_win,freq); tr=generate_trades(prices,sig,top_n,freq)
        return tr if tr is not None and not tr.empty else pd.DataFrame()
...
def combo_to_params(c): tp,freq,mw=c; return {"top_pct":float(tp),"freq":freq,"mom_window":int(mw)}
...
print("\n  Top-5:"); 
for rk,(ci,med) in enumerate(ranked[:5],1):
    p=combo_to_params(ALL_COMBOS[ci]); print(f"  {rk}  med={med:.3f}  {p}")
        ...
        bonferroni_rescue(
            ...
            monthly_returns_fn=lambda _t, _c: compute_monthly_returns(
                build_daily_equity_from_trades(
                    ticker_data[_t]["close"], generate_commodity_trend_trades  # TODO: verify fn name(ticker_data[_t], _t, *_c))),
            out_path=os.path.join(RESULTS_DIR, "commodity_trend_v2_bonferroni_results.json"),
        )
    else:
