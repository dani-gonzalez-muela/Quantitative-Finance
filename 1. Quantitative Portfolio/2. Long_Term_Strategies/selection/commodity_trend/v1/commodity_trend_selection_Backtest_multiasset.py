# -*- coding: utf-8 -*-
"""
Commodity Trend — Multi-Asset Selection Backtest
================================================
Adapts commodity_trend_selection_backtest.py to ETF universes.
Original signal: 12m return > 0 → long; else flat (binary trend-follow).
Multiasset adaptation: rank commodity/real-asset ETFs by N-month return,
hold top_pct fraction.
...
Outputs
-------
  results/commodity_trend_multiasset_grid.csv
  results/commodity_trend_multiasset_summary.json
"""
...
STRATEGY_NAME    = "Commodity Trend (Multi-Asset)"
SAVE_NAME        = "commodity_trend_multiasset"
...
print("=" * 75)
print("COMMODITY TREND — Multi-Asset Backtest")
print("=" * 75)
...
lookup  = {(r["top_pct"], r["freq"], r["mom_window"]): r["median_sharpe"] for _, r in grid_df.iterrows()}
stab    = []
...
grid_df[[c for c in col_order if c in grid_df.columns]].sort_values(
    "median_sharpe", ascending=False).to_csv(os.path.join(RESULTS_DIR, "commodity_trend_multiasset_grid.csv"), index=False)
...
with open(os.path.join(RESULTS_DIR, "commodity_trend_multiasset_summary.json"), "w") as f:
...
for _, _rr in _top3.iterrows():
    _tp = float(_rr["top_pct"]); _freq = _rr["freq"]; _mw = int(_rr["mom_window"])
    ...
    for _uni, _prices in UNIVERSES.items():
        ...
        if _ck not in _pret_cache: _pret_cache[_ck] = get_period_returns(_prices, _tp, _freq, _mw)
