...
# -*- coding: utf-8 -*-
"""Country CAPE Rotation — Implementation v2 (TYPE 2 SELECTION). Uses build_basket_equity."""
import sys, os
...
STRATEGY_NAME="country_cape_rotation_v2"; STARTING_CAPITAL=100_000
...
SUMMARY_PATH=os.path.join(RESULTS_DIR,"country_cape_rotation_v2_multiasset_summary.json")
...
print("="*70); print("COUNTRY CAPE ROTATION — Implementation v2  (TYPE 2 SELECTION)"); print("="*70)
...
for bname,cfg in passing.items():
    cp=cfg["canonical_params"]; top_pct=float(cp["top_pct"]); freq=cp["freq"]; rw=int(cp["return_window"])
    tickers=UNIVERSE_SPECS.get(bname,cfg.get("instruments",[]))
    prices=load_prices(tickers)
    if prices.empty: continue
    top_n=max(1,round(len(prices.columns)*top_pct)); sig=compute_contrarian_signal(prices,rw,freq)
    trades=generate_trades(prices,sig,top_n,freq)
    print(f"  BASKET: {bname}  top_n={top_n} rw={rw} {freq}  {len(trades)} trades")
...
jp=os.path.join(RESULTS_DIR,"country_cape_rotation_v2_implementations_multiasset.json")
