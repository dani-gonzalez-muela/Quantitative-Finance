...
# -*- coding: utf-8 -*-
"""
Country CAPE Rotation — Multi-Asset Implementation
===================================================
Canonical: top_pct=0.10, return_window=9, freq=ME
Basket   : em_country

Contrarian signal: score = -trailing_return (lowest return = "cheapest").
Proxy for Shiller CAPE since country-level P/E data unavailable for ETFs.

Outputs: results/country_cape_rotation_multiasset_daily_equity/*.csv
         results/country_cape_rotation_implementations_multiasset.json
"""
import sys, os
...
STRATEGY_NAME    = "country_cape_rotation"
...
SUMMARY_PATH = os.path.join(RESULTS_DIR, "country_cape_rotation_multiasset_summary.json")
...
print("="*70); print("COUNTRY CAPE ROTATION — Multi-Asset Implementation"); print("="*70)

with open(SUMMARY_PATH) as f: summary = json.load(f)
best = summary["best_params"]
CANON_TOP_PCT  = float(best["top_pct"])
CANON_RET_WIN  = int(best["return_window"])
CANON_FREQ     = best["freq"]
PASSING_BASKETS = list(summary["universes"].keys())
print(f"\nCanonical: top_pct={CANON_TOP_PCT}, ret_win={CANON_RET_WIN}, freq={CANON_FREQ}")
...
def compute_contrarian_signal(prices_wide, ret_window, freq):
    """score = -trailing_return (lowest = cheapest = highest contrarian score)."""
    monthly = prices_wide.resample(freq).last()
    fmon = _FREQ_MONTHS.get(freq,1)
    wper = max(1, int(round(ret_window/fmon)))
    skip = 1
    ...
    for i in range(len(monthly)):
        ...
        sig.iloc[i] = -ret  # contrarian: inverse momentum
    return sig
...
for basket_name in PASSING_BASKETS:
    ...
    sig = compute_contrarian_signal(prices, CANON_RET_WIN, CANON_FREQ)
...
out = {"strategy":STRATEGY_NAME,"period":f"{START_DATE} → {END_DATE}","tc_bps_one_way":TC_BPS_OW,
       "canonical_params":{"top_pct":CANON_TOP_PCT,"return_window":CANON_RET_WIN,"freq":CANON_FREQ},
       "sleeves":{b:{v:d["metrics"] for v,d in vd.items()} for b,vd in sleeve_results.items()},
       "combined":combined_results}
jp = os.path.join(RESULTS_DIR, "country_cape_rotation_implementations_multiasset.json")
