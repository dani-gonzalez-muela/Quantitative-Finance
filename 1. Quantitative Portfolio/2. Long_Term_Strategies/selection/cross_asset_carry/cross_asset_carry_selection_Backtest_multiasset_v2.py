...
# -*- coding: utf-8 -*-
"""Cross-Asset Carry — Multi-Asset Selection Backtest v2
Full grid. Canonical=highest median Sharpe. Binomial test. n_boot=500.
Outputs: results/cross_asset_carry_v2_multiasset_summary.json
"""
import sys, os
...
STRATEGY_NAME="Cross-Asset Carry v2 (Multi-Asset)"; SAVE_NAME="cross_asset_carry"
...
MIN_TICKERS=5
TOP_PCTS=[0.10,0.15,0.25,0.33,0.50,1.00]; REBAL_FREQS=["ME","2ME","QE","6ME","12ME"]; CARRY_WINDOWS=[3,6,9,12]
ALL_COMBOS=list(itertools.product(TOP_PCTS,REBAL_FREQS,CARRY_WINDOWS))
...
def compute_carry_signal(prices,carry_win,freq):
    """Carry = trailing N-month return / trailing N-month vol (Sharpe-like)."""
    ...
    for i in range(len(monthly)):
        ...
        carry=(ret/vol.replace(0,np.nan)).fillna(0)
        sig.iloc[i]=carry
    return sig
...
def run_trades(prices,combo):
    top_pct,freq,cw=combo; top_n=max(1,round(len(prices.columns)*top_pct))
    try:
        sig=compute_carry_signal(prices,cw,freq); tr=generate_trades(prices,sig,top_n,freq)
...
def combo_to_params(c): tp,freq,cw=c; return {"top_pct":float(tp),"freq":freq,"carry_window":int(cw)}
        ...
        bonferroni_rescue(
            ...
            monthly_returns_fn=lambda _t, _c: compute_monthly_returns(
                build_daily_equity_from_trades(
                    ticker_data[_t]["close"], generate_cross_asset_carry_trades  # TODO: verify fn name(ticker_data[_t], _t, *_c))),
            out_path=os.path.join(RESULTS_DIR, "cross_asset_carry_v2_bonferroni_results.json"),
        )
    else:
