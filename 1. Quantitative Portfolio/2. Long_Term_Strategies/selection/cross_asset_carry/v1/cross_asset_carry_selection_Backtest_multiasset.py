# -*- coding: utf-8 -*-
"""
Cross-Asset Carry — Multi-Asset Selection Backtest
===================================================
Adapts cross_asset_carry_selection_backtest.py to ETF universes.
Original signal: carry proxy = trailing_12m_return / trailing_12m_vol
                 (Sharpe-like carry), rank and hold top 3 of 5 assets.
Multiasset adaptation: same carry signal on a mixed basket of
bonds_us + commodities + us_equity_broad ETFs.

Universe
--------
  cross_asset  — bonds_us + commodities + us_equity_broad (mixed basket)
  ["SPY","QQQ","IWM","MDY",       (us_equity_broad)
   "TLT","IEF","SHY","TIP","HYG", (bonds_us subset)
   "GLD","SLV","DBC","GDX"]       (commodities subset)

Grid
----
  TOP_PCTS      = [0.10, 0.15, 0.25, 0.33, 0.50, 1.00]
  REBAL_FREQS   = ['ME', '2ME', 'QE', '6ME', '12ME']
  CARRY_WINDOWS = [3, 6, 9, 12]   (months lookback for return + vol)
  = 120 combos × 1 universe = 120 total runs

Outputs
-------
  results/cross_asset_carry_multiasset_grid.csv
  results/cross_asset_carry_multiasset_summary.json
"""
...
STRATEGY_NAME    = "Cross-Asset Carry (Multi-Asset)"
SAVE_NAME        = "cross_asset_carry_multiasset"
...
# Mixed basket across asset classes — tests if carry generalises cross-asset
UNIVERSE_SPECS = {
    "cross_asset": [
        "SPY","QQQ","IWM","MDY",          # us_equity_broad
        "TLT","IEF","SHY","TIP","HYG",    # bonds_us
        "GLD","SLV","DBC","GDX",          # commodities
    ],
}
MIN_TICKERS = 5
...
TOP_PCTS      = [0.10, 0.15, 0.25, 0.33, 0.50, 1.00]
REBAL_FREQS   = ["ME", "2ME", "QE", "6ME", "12ME"]
CARRY_WINDOWS = [3, 6, 9, 12]   # months — lookback for return AND vol
_FREQ_MONTHS  = {"ME": 1, "2ME": 2, "QE": 3, "6ME": 6, "12ME": 12}

print("=" * 75)
print("CROSS-ASSET CARRY — Multi-Asset Backtest")
print("=" * 75)
...
def compute_carry_signal(prices_wide, carry_window, rebalance_freq):
    """
    Carry proxy = N-month return / N-month volatility (Sharpe ratio as carry).
    Uses monthly resampled prices.
    """
    monthly = prices_wide.resample(rebalance_freq).last()
    months_per_period = _FREQ_MONTHS.get(rebalance_freq, 1)
    window_periods = max(2, int(round(carry_window / months_per_period)))

    signal_df = pd.DataFrame(index=monthly.index, columns=monthly.columns, dtype=float)
    for i in range(len(monthly)):
        if i < window_periods + 1:
            continue
        past  = monthly.iloc[i - window_periods - 1]
        now   = monthly.iloc[i - 1]
        ret_n = now / past - 1
        # Monthly returns over window
        rets  = monthly.iloc[i - window_periods: i].pct_change().dropna()
        vol_n = rets.std() * np.sqrt(12 / months_per_period) + 1e-9
        signal_df.iloc[i] = ret_n / vol_n
    return signal_df
...
def run_single(prices, top_pct, freq, carry_win):
    n = len(prices.columns); top_n = max(1, round(n * top_pct))
    try:
        sig    = compute_carry_signal(prices, carry_win, freq)
...
all_combos = list(product(TOP_PCTS, REBAL_FREQS, CARRY_WINDOWS))
...
results_raw = []
for idx, (top_pct, freq, carry_win) in enumerate(all_combos, 1):
    row = {"top_pct": top_pct, "freq": freq, "carry_window": carry_win}
    sharpes = []
    for uni_name, prices in UNIVERSES.items():
        s = run_single(prices, top_pct, freq, carry_win)
        row[uni_name] = s
...
lookup  = {(r["top_pct"], r["freq"], r["carry_window"]): r["median_sharpe"] for _, r in grid_df.iterrows()}
stab    = []
for _, row in grid_df.iterrows():
    nbrs = []
    for di in [-1,1]:
        ni = TOP_PCTS.index(row["top_pct"]) + di
        if 0<=ni<len(TOP_PCTS): nbrs.append((TOP_PCTS[ni], row["freq"], row["carry_window"]))
    for dj in [-1,1]:
        nj = REBAL_FREQS.index(row["freq"]) + dj
        if 0<=nj<len(REBAL_FREQS): nbrs.append((row["top_pct"], REBAL_FREQS[nj], row["carry_window"]))
    for dk in [-1,1]:
        nk = CARRY_WINDOWS.index(row["carry_window"]) + dk
        if 0<=nk<len(CARRY_WINDOWS): nbrs.append((row["top_pct"], row["freq"], CARRY_WINDOWS[nk]))
...
col_order = ["top_pct","top_n","freq","carry_window","median_sharpe","min_sharpe","n_positive","stability"] + uni_names
grid_df[[c for c in col_order if c in grid_df.columns]].sort_values(
    "median_sharpe", ascending=False).to_csv(os.path.join(RESULTS_DIR, "cross_asset_carry_multiasset_grid.csv"), index=False)
...
summary = {
    ...
    "grid_search": {"top_pcts": TOP_PCTS, "rebal_freqs": REBAL_FREQS, "carry_windows": CARRY_WINDOWS,
    ...
    "best_params": {
        ...
        "carry_window": int(best["carry_window"]), "freq": best["freq"],
...
with open(os.path.join(RESULTS_DIR, "cross_asset_carry_multiasset_summary.json"), "w") as f:
...
def get_period_returns(prices, top_pct, freq, carry_win):
    n = len(prices.columns); top_n = max(1, round(n*top_pct))
    try:
        sig    = compute_carry_signal(prices, carry_win, freq)
        trades = generate_trades(prices, sig, top_n, freq)
...
for _, _rr in _top3.iterrows():
    _tp = float(_rr["top_pct"]); _freq = _rr["freq"]; _cw = int(_rr["carry_window"])
    print(f"\n  -- top_pct={_tp:.2f}  carry_win={_cw}  {_freq} --")
    print("  {:<15}  {:>7}  {:>7}  {:>7}  {:>8}  {:>7}  Score".format("Universe","Sharpe","t-stat","t-p","boot5%","perm-p"))
    print("  "+"-"*71)
    for _uni, _prices in UNIVERSES.items():
        _ck = (_tp, _freq, _cw, _uni)
        if _ck not in _pret_cache: _pret_cache[_ck] = get_period_returns(_prices, _tp, _freq, _cw)
