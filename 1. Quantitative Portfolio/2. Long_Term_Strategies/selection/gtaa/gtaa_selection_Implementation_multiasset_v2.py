...
# -*- coding: utf-8 -*-
"""GTAA — Implementation v2 (TYPE 2 SELECTION). Uses build_basket_equity."""
import sys, os
...
STRATEGY_NAME="gtaa_v2"; STARTING_CAPITAL=100_000
...
SUMMARY_PATH=os.path.join(RESULTS_DIR,"gtaa_v2_multiasset_summary.json")
...
print("="*70); print("GTAA — Implementation v2  (TYPE 2 SELECTION)"); print("="*70)
...
# Filter: strategy must pass overall binomial AND per-basket >=2/3 gates
...
def compute_sma_signal(prices,sma_win,freq,top_pct):
    ...
    for rd in rebal_dates:
        ...
        excess=(last/sma-1).where(last>sma,0.0); pos=excess[excess>0]
...
for bname,cfg in passing.items():
    cp=cfg["canonical_params"]; sma_win=int(cp["sma_window"]); freq=cp["freq"]; top_pct=float(cp["top_pct"])
    tickers=UNIVERSE_SPECS.get(bname,cfg.get("instruments",[]))
    prices=load_prices(tickers)
    if prices.empty: continue
    sig=compute_sma_signal(prices,sma_win,freq,top_pct); trades=generate_trades(prices,sig,freq)
    print(f"  BASKET: {bname}  sma={sma_win} top_pct={top_pct} {freq}  {len(trades)} trades")
...
jp=os.path.join(RESULTS_DIR,"gtaa_v2_implementations_multiasset.json")
