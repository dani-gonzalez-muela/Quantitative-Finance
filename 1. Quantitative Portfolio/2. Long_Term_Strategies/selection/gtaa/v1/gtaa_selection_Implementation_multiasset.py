...
# -*- coding: utf-8 -*-
"""
GTAA — Multi-Asset Implementation
===================================
Canonical: top_pct=0.25, sma_window=300, freq=6ME
Baskets  : us_sectors, bonds_us, commodities, real_assets

SMA signal: hold ETF if close > SMA_N; rank by (close/sma - 1).

Outputs: results/gtaa_multiasset_daily_equity/*.csv
         results/gtaa_implementations_multiasset.json
"""
import sys, os
...
SUMMARY_PATH = os.path.join(RESULTS_DIR, "gtaa_multiasset_summary.json")
...
def compute_sma_signal(prices_wide, sma_window, freq):
    """
    Score = close/sma - 1 when close > SMA, else NaN (ETF not eligible).
    top_pct is applied only among eligible (above SMA) ETFs.
    """
    sma = prices_wide.rolling(sma_window, min_periods=sma_window//2).mean()
    above_sma = prices_wide > sma
    score = (prices_wide / sma - 1).where(above_sma)
    signal_df = score.resample(freq).last()
    return signal_df
...
for basket_name in PASSING_BASKETS:
    ...
    sig = compute_sma_signal(prices, CANON_SMA_WIN, CANON_FREQ)
    trades = generate_trades(prices, sig, top_n, CANON_FREQ)
    if trades.empty: print(f"  {basket_name}: no trades (SMA window may exceed history)"); continue
...
for var_name, series_list in combined_daily.items():
    ...
    passing_in_output = [b for b in PASSING_BASKETS if b in sleeve_results]
    combined_results[var_name] = {"n_sleeves":n_sl,"sleeves":passing_in_output,**st}
...
out = {"strategy":STRATEGY_NAME,"period":f"{START_DATE} → {END_DATE}","tc_bps_one_way":TC_BPS_OW,
       "canonical_params":{"top_pct":CANON_TOP_PCT,"sma_window":CANON_SMA_WIN,"freq":CANON_FREQ},
       "sleeves":{b:{v:d["metrics"] for v,d in vd.items()} for b,vd in sleeve_results.items()},
       "combined":combined_results}
jp = os.path.join(RESULTS_DIR, "gtaa_implementations_multiasset.json")
