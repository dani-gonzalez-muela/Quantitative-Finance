...
# -*- coding: utf-8 -*-
"""Sector Momentum — Multi-Asset Selection Backtest v2
Full grid. Canonical=highest median Sharpe across baskets. Binomial test. n_boot=500.
Outputs: results/sector_momentum_v2_multiasset_summary.json
"""
import sys, os
...
STRATEGY_NAME="Sector Momentum v2 (Multi-Asset)"; SAVE_NAME="sector_momentum"
...
def run_trades(prices,combo):
    top_pct,freq,mw=combo; top_n=max(1,round(len(prices.columns)*top_pct))
    try:
        sig=compute_momentum(prices,mw,freq); return generate_trades(prices,sig,top_n,freq)
        ...
        bonferroni_rescue(
            ...
            monthly_returns_fn=lambda _t, _c: compute_monthly_returns(
                build_daily_equity_from_trades(
                    ticker_data[_t]["close"], generate_sector_momentum_trades  # TODO: verify fn name(ticker_data[_t], _t, *_c))),
            out_path=os.path.join(RESULTS_DIR, "sector_momentum_v2_bonferroni_results.json"),
        )
    else:
