...
# -*- coding: utf-8 -*-
"""Sector Momentum — Implementation v2 (TYPE 2 SELECTION). Uses build_basket_equity."""
import sys, os
...
STRATEGY_NAME="sector_momentum_v2"; STARTING_CAPITAL=100_000
...
SUMMARY_PATH=os.path.join(RESULTS_DIR,"sector_momentum_v2_multiasset_summary.json")
...
print("="*70); print("SECTOR MOMENTUM - Implementation v2  (TYPE 2 SELECTION)"); print("="*70)
...
passing={n:c for n,c in basket_cfgs.items()
         if c.get("binomial_significant",False)
         and c.get("basket_details",{}).get(n,{}).get("gates",0)>=2}
print(f"  Baskets: {len(basket_cfgs)}  Passing: {len(passing)}")
for n,c in basket_cfgs.items():
    gates=c.get("basket_details",{}).get(n,{}).get("gates",0)
    ok=c.get("binomial_significant",False) and gates>=2
    print(f"    [{'PASS' if ok else 'FAIL'}] {n}  med={c['canonical_median_sharpe']:.3f}  gates={gates}/3")
...
def compute_momentum(prices,mom_win,freq):
    monthly=prices.resample(freq).last(); fm=_FREQ_MONTHS.get(freq,1); wper=max(1,int(round(mom_win/fm)))
    ...
    for i,rd in enumerate(rdates):
        ...
        for sym in tops:
            ...
            trades.append({"entry_time":ed,"exit_time":xd,"direction":"long","instrument":sym,
                           ...
                           "pct_return_gross":round(float((xp-ep)/ep),6),"exit_reason":xr,"stop_price":float("nan")})
...
for bname,cfg in passing.items():
    ...
    top_n=max(1,round(len(prices.columns)*top_pct)); sig=compute_momentum(prices,mom_win,freq)
    trades=generate_trades(prices,sig,top_n,freq)
    print(f"  BASKET: {bname}  top_n={top_n} mom={mom_win}m {freq}  {len(trades)} trades")
...
jp=os.path.join(RESULTS_DIR,"sector_momentum_v2_implementations_multiasset.json")
with open(jp,"w") as f:
    json.dump({"strategy":STRATEGY_NAME,"period":f"{START_DATE} to {END_DATE}","tc_bps_one_way":TC_BPS_OW,
