...
"""
Bond Duration Carry — Implementation (single-instrument stub)
...
Signal: regime switch: long/mid/short duration based on DFII10 real yield
Instrument: bond_regime

What this file CAN do:
- Test EXPOSURE LEVEL variants (conservative / baseline / aggressive)
  by loading results/bond_duration_carry_trades.csv and calling shared.implementations.
- For parameter sweeps (e.g. different VIX thresholds, holding periods),
  modify bond_duration_carry_Backtest.py and re-run.

To run full sizing comparison, see:
    shared.implementations.simple_bet / vol_targeting / risk_based
    (already applied in bond_duration_carry_Backtest.py results)
"""
import sys, os
...
SAVE_NAME        = "bond_duration_carry"
STRATEGY_NAME    = "Bond Duration Carry"
