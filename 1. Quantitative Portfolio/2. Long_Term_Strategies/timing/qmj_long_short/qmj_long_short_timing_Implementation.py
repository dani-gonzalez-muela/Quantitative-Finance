...
"""
QMJ - Quality Minus Junk — Implementation (single-instrument stub)
...
Signal: long/short market-neutral via FF RMW+CMA factor; no instrument selection
Instrument: FF_RMW_CMA_proxy

What this file CAN do:
- Test EXPOSURE LEVEL variants (conservative / baseline / aggressive)
  by loading results/qmj_long_short_trades.csv and calling shared.implementations.
- For parameter sweeps (e.g. different VIX thresholds, holding periods),
  modify qmj_long_short_Backtest.py and re-run.

To run full sizing comparison, see:
    shared.implementations.simple_bet / vol_targeting / risk_based
    (already applied in qmj_long_short_Backtest.py results)
"""
import sys, os
...
SAVE_NAME        = "qmj_long_short"
STRATEGY_NAME    = "QMJ - Quality Minus Junk"
