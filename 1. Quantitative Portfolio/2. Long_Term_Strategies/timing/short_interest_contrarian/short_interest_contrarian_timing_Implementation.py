...
"""
Short Interest Contrarian — Implementation (single-instrument stub)
...
Signal: monthly composite (FF Mkt+0.3*SMB proxy); individual stock scores not in signal.csv
Instrument: LowShortInterest_quintile

What this file CAN do:
- Test EXPOSURE LEVEL variants (conservative / baseline / aggressive)
  by loading results/short_interest_contrarian_trades.csv and calling shared.implementations.
- For parameter sweeps (e.g. different VIX thresholds, holding periods),
  modify short_interest_contrarian_Backtest.py and re-run.

To run full sizing comparison, see:
    shared.implementations.simple_bet / vol_targeting / risk_based
    (already applied in short_interest_contrarian_Backtest.py results)
"""
import sys, os
...
SAVE_NAME        = "short_interest_contrarian"
STRATEGY_NAME    = "Short Interest Contrarian"
