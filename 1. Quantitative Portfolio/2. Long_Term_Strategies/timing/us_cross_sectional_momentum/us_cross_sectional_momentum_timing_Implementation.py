...
"""
US Cross-Sectional Momentum — Implementation (single-instrument stub)
...
Signal: FF factor composite (Mkt+0.5*Mom); no individual stock selection
Instrument: US_TopDecile_Momentum

What this file CAN do:
- Test EXPOSURE LEVEL variants (conservative / baseline / aggressive)
  by loading results/us_cross_sectional_momentum_trades.csv and calling shared.implementations.
- For parameter sweeps (e.g. different VIX thresholds, holding periods),
  modify us_cross_sectional_momentum_Backtest.py and re-run.

To run full sizing comparison, see:
    shared.implementations.simple_bet / vol_targeting / risk_based
    (already applied in us_cross_sectional_momentum_Backtest.py results)
"""
import sys, os
