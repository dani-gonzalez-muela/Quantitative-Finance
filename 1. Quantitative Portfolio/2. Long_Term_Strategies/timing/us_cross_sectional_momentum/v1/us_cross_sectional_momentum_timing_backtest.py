"""
Strategy 1: US Cross-Sectional Momentum
Data: Fama-French factors (ff_factors_monthly.csv) — Mom (UMD) factor.
Long-only portfolio = Mkt + 0.5*Mom, representing top-decile momentum tilt.
Academic basis: Jegadeesh & Titman (1993); FF Mom factor = long top 30%, short bottom 30%.
Long-only top-decile return ≈ Mkt + 0.5*Mom (since market ≈ avg of high + low mom stocks).
Data note: Jensen/Kelly (68GB uncompressed) exceeds RAM; FF factors used as standard proxy.
"""
import sys, os, json
...
SAVE_NAME = "us_cross_sectional_momentum"
STRATEGY_NAME = "US Cross-Sectional Momentum"
...
# Long-only momentum portfolio: Mkt + 0.5*Mom tilt (approx top-decile long leg)
# TC: monthly rebalance costs ~10bps round-trip
ret_g = mkt + 0.5 * mom
...
def sig(r):
    ...
    p = sum([t1,t2,t3])
    v = "SIGNIFICANT (strong)" if p==3 else "SIGNIFICANT (moderate)" if p==2 else "NOT SIGNIFICANT"
    return {"sharpe":round(float(sh),4),"verdict":v,"tests_passed":f"{p}/3"}
...
# Trades log
trades = [{"entry_time": ret_g.index[i-1] if i>0 else ret_g.index[0],
           "exit_time": ret_g.index[i], "direction":"long",
           "instrument":"US_TopDecile_Momentum","entry_price":100.0,
           "exit_price":round(100*(1+float(ret_g.iloc[i])),4),
...
summary = {"strategy":STRATEGY_NAME,"instruments":["FF_Mkt","FF_Mom"],
  "portfolio":"long_term",
  "period":f"{daily_eq.index[0].strftime('%Y-%m-%d')} -> {daily_eq.index[-1].strftime('%Y-%m-%d')}",
  "params":{"signal":"ret_12_1 (12-1 month momentum)",
            "implementation":"Mkt+0.5*Mom (FF factor long-only proxy for top-decile momentum)",
            "universe":"US stocks (FF universe)","tc_bps":"5 one-way",
            "data_notes":"Jensen/Kelly 68GB dataset exceeds RAM; FF Mom factor used as standard proxy per Jegadeesh & Titman (1993)"},
...
print("Done:", RESULTS_DIR)


# ── Save signal.csv for us_cross_sectional_momentum_Implementation.py ──
...
signal_data = pd.DataFrame({
    ...
    "instrument": "US_TopDecile_Momentum",
    "score": 1.0,
})
