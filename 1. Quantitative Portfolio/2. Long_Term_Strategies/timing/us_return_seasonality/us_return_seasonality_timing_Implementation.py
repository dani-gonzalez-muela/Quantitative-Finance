...
"""
US Return Seasonality — Implementation (single-instrument stub)
...
Signal: market exposure tilt (0.8x/1.2x) based on calendar-month seasonal avg
Instrument: US_Market_SeasonalTilt

What this file CAN do:
- Test EXPOSURE LEVEL variants (conservative / baseline / aggressive)
  by loading results/us_return_seasonality_trades.csv and calling shared.implementations.
- For parameter sweeps (e.g. different VIX thresholds, holding periods),
  modify us_return_seasonality_Backtest.py and re-run.

To run full sizing comparison, see:
    shared.implementations.simple_bet / vol_targeting / risk_based
    (already applied in us_return_seasonality_Backtest.py results)
"""
import sys, os
...
SAVE_NAME        = "us_return_seasonality"
STRATEGY_NAME    = "US Return Seasonality"
