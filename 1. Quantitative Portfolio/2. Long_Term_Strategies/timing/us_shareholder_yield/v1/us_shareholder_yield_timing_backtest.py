"""
Strategy 2: US Shareholder Yield
Signal: div12m_me (dividend yield). Proxy: FF HML factor (high dividend yield ≈ high book-to-market).
Long-only portfolio = Mkt + 0.5*HML, representing top-quintile dividend yield stocks.
Reference: Meb Faber "Shareholder Yield" (2013); Fama & French (1988).
Data note: Jensen/Kelly 68GB exceeds RAM; FF HML used as dividend-yield proxy.
"""
import sys, os, json
...
SAVE_NAME = "us_shareholder_yield"
STRATEGY_NAME = "US Shareholder Yield"
...
mkt = ff["Mkt-RF"] + ff["RF"]; hml = ff["HML"]; rf = ff["RF"]

# Long-only high dividend yield ≈ Mkt + 0.5*HML (value tilt, correlated with yield)
ret_g = (mkt + 0.5 * hml).dropna()
...
trades=[{"entry_time":ret_g.index[i-1] if i>0 else ret_g.index[0],"exit_time":ret_g.index[i],
         "direction":"long","instrument":"US_TopQuintile_DivYield","entry_price":100.0,
...
summary={"strategy":STRATEGY_NAME,"instruments":["FF_Mkt","FF_HML"],"portfolio":"long_term",
  "period":f"{daily_eq.index[0].strftime('%Y-%m-%d')} -> {daily_eq.index[-1].strftime('%Y-%m-%d')}",
  "params":{"signal":"div12m_me (dividend yield)","implementation":"Mkt+0.5*HML (FF value factor proxy for high-yield)","tc_bps":"5 one-way",
            "data_notes":"FF HML used as proxy; HML long leg = high book-to-market stocks which historically have high dividend yields"},
...
# ── Save signal.csv for us_shareholder_yield_Implementation.py ──
...
signal_data = pd.DataFrame({
    ...
    "instrument": "US_TopQuintile_DivYield",
    "score": 1.0,
})
