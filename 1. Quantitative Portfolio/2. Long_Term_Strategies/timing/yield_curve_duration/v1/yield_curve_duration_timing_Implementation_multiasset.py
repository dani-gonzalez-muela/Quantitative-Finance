...
# -*- coding: utf-8 -*-
"""
Yield Curve Duration - Multi-Asset Implementation
Passing instruments (3/3 gates): SHY, BIL
Signal: T10Y2Y < 0% -> short-duration regime; hold SHY / BIL.
...
STRATEGY_NAME  = "yield_curve_duration"
...
SUMMARY_PATH   = os.path.join(RESULTS_DIR, "yield_curve_duration_multiasset_summary.json")
os.makedirs(EQUITY_DIR, exist_ok=True)

print("=" * 70)
print("YIELD CURVE DURATION -- Multi-Asset Implementation")
print("=" * 70)

with open(SUMMARY_PATH) as f:
    summary = json.load(f)

PASSING = summary["passing_instruments"]   # ['SHY', 'BIL']
print("\nPassing instruments: %s" % PASSING)
print("Signal: T10Y2Y lagged 1m; hold ETF when regime=='short' (T10Y2Y < 0%)\n")
...
def regime_for(dt):
    v = spread_lag.get(dt, float("nan"))
    if pd.isna(v):   return "unknown"
    if v > 0.5:      return "long"
...
out = {
    ...
    "signal":            "T10Y2Y lagged 1m; hold when regime=='short' (T10Y2Y < 0%)",
    "passing_instruments": PASSING,
    "sleeves":           {t: {v: d["metrics"] for v, d in vd.items()}
                          for t, vd in sleeve_results.items()},
    "combined":          combined_results,
}
jp = os.path.join(RESULTS_DIR, "yield_curve_duration_implementations_multiasset.json")
with open(jp, "w") as f:
